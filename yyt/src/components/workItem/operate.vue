<template>
  <section>
    ...
    <div class="pageinfo-tab">
      <ul class="clearfix">
        <!--<li v-for="(item,index) in tabToggleData" :class="item.display ? 'active' : ''" @click="tabToggleClick(index)">{{item.title}}</li>-->
      </ul>
    </div>
...
<script>
  ...
  import {requestDayData} from '../../axios/axios';
  import {requestMonthData} from '../../axios/axios';
...
//            console.log(res)
        ...
        requestDayData(params4).then(res => {
//            console.log(res)
          this.query_day_recharge_amount = res.data[0].Amount?res.data[0].Amount:0
        });
        let params5 = {
          'group_id': this.$store.state.loginInfo.group_id,
          "buis_type": "busi",
        ...
        requestDayData(params5).then(res => {
//            console.log(res)
          this.query_day_cancel_amount = res.data[0].Amount?res.data[0].Amount:0
        });
...
//        console.log(preMonth-1);
        if (thisDate == 1) {
          ...
          requestMonthData(params4).then(res => {
            this.query_month_recharge_amount = res.data[0].Amount?res.data[0].Amount:0;
          });
          let params5 = {
            'group_id': this.$store.state.loginInfo.group_id,
            "buis_type": "busi",
          ...
          requestMonthData(params5).then(res => {
            this.query_month_cancel_amount = res.data[0].Amount?res.data[0].Amount:0;
          });
        }
